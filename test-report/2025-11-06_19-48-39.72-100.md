...
- ✅ Add fake timers/RNG in test setup
- ✅ Clean `.only`/`.skip` tests
- ✅ Add ESLint `no-only-tests`, `no-restricted-imports` (fs, fetch)
- ✅ Rewrite 10 weakest tests (see Section C)
- **Exit Criteria:** Mutation score ≥70%, no flaky tests in CI

**60 Days (Architecture):**
- ✅ Create contract suites for 3 seams: `GitPlatformAdapter`, `HookExecutor`, `DetectorService`
- ✅ Add property tests for pure functions: artifact ID extraction, branch validation, cascade logic
- ✅ Build test data builders (`MergeMetadataBuilder`, `OrchestrationResultBuilder`)
- **Exit Criteria:** Contract tests run against real & fake adapters; property tests cover 5+ invariants

**90 Days (Culture & Gates):**
- ✅ CI gates: diff coverage ≥80%, mutation score ≥75% (core modules ≥85%)
- ✅ Flake budget policy: <2% failure rate, auto-quarantine flaky tests
- ✅ Remove git integration tests or move to separate `e2e/` suite
- ✅ Document test philosophy: "Test invariants, not implementation"
- **Exit Criteria:** Zero `.only`/`.skip` in main; <5% mock ratio in unit tests; mutation score ≥80%
...
**Worst Example:** [strategy-executor.test.ts:186-209](src/hooks/orchestration/strategy-executor.test.ts#L186-L209) - 6-deep mock chain (git log → checkout → add → commit → rev-parse → push) tests nothing about domain logic.
...
### 1. **Shallow Success Check → Invariant Test**
...
### 2. **Over-Mocked Private Method → Fake Adapter**
...
class FakeGitAdapter {
  branches: Map<string, string[]> = new Map(); // branch → commits
  prs: PRInfo[] = [];
...
class CascadeResultBuilder {
  ...
  withCompletedArtifacts(...ids: string[]): this {
    this.result.completionCascade.events = ids.map(id =>
      event(CArtifactEvent.COMPLETED, id)
    );
    return this;
  }
...
### 4. **Real Time Dependency → Injected Clock**
...
### 5. **Weak Assertion → Semantic Check**
...
7. **Branch Validator:** Add property test: `∀ branchName: extractArtifactIds(branchName) produces sorted unique list`.
...
export function contractHookExecutor(
  ...
  describe('HookExecutor contract', () => {
    ...
    it('lifecycle callbacks fire in order: before → (execute) → after OR onError', async () => {
...
|--------|------------------|----------|-----------------|-------------|------------|--------|
| **orchestration/** | 🔥 **10,000+ LOC** | 85% | ⚠️ ~50% | C+ | **CRITICAL** | Rewrite with FakeGitAdapter; add contract tests |
| **detection/** | 🔥 3,500 LOC | 92% | ~70% | B | HIGH | Add property tests for regex; extract repo builder |
| **core/** | 🔥 2,800 LOC | 95% | ~75% | B+ | MEDIUM | Extract fake logger; add lifecycle invariant tests |
| **validation/** | 600 LOC | 94% | ~80% | B | LOW | Add property tests for edge cases |
| **cascade/** | 🔥 900 LOC | 88% | ~60% | B- | MEDIUM | Add builder; test commit message formatting |
| **adapters/** | 1,200 LOC | 82% | ⚠️ ~40% | C | HIGH | Extract to contract tests; reduce integration tests |
| **draft-pr/** | 400 LOC | 90% | ~65% | B | LOW | Minor: strengthen error assertions |

**Legend:**
- 🔥 High churn = Added/changed in last 30 days
- Risk Score = (Churn Percentile) × (1 - Mutation Score) × (1 - Qual. Grade / 100)
...
### 2. Mock Ratio
**Finding:** 108 mocks / ~240 tests = **45% mock ratio** (threshold: 40%). ❌
...
### 3. Timer/RNG Usage
**Finding:** Zero `useFakeTimers` or `Math.random` mocks. ❌
...
- Semantic (state/output): ~400 (66%) ✅
- Structural (`toBeDefined`, `toBeGreaterThanOrEqual(0)`): ~150 (25%) ⚠️
- Mock verification (`toHaveBeenCalledWith`): ~59 (10%) ⚠️
...
- **Orchestration tests:** 45 lines ❌
- Core tests: 15 lines ⚠️
...
**Total integration time:** 14.4s / 15s = **96% of test time** ❌
...
**Health:** ⚠️ Moderate - Integration tests dominate. Move to separate suite.
...
1. **Vitest Config:** ✅ Clean, has coverage thresholds (90/95/80/90)
2. **Coverage Provider:** ✅ Istanbul (good choice)
3. **No Stryker Config:** ❌ **CRITICAL GAP**
4. **No ESLint test rules:** ❌ Missing (see Section D.1)
5. **No test/setup.ts:** ❌ Missing (see Section D.2)
6. **CI Coverage Gates:** ✅ (inferred from thresholds in vitest.config)
